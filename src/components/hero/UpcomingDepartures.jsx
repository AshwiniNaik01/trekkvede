...
const UpcomingDepartures = () => {
  ...
  const slideTransition = (targetSlide, direction = 'next') => {
    if (isAnimating || targetSlide === activeSlide) return;
    
    setIsAnimating(true);
    setSlideDirection(direction);
    
    // Set active slide after a small delay for animation start
    setTimeout(() => {
  ...
  const prevSlide = () => {
    const prev = (activeSlide - 1 + departures.length) % departures.length;
...
useEffect(() => {
  ...
  return (
    <section className="py-7 bg-gradient-to-b from-gray-50 to-emerald-50 relative overflow-hidden">
      {/* Background Elements */}
      <div className="absolute inset-0">
        <div className="absolute top-1/4 left-1/4 w-64 h-64 bg-emerald-200/20 rounded-full blur-3xl"></div>
        <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-amber-200/10 rounded-full blur-3xl"></div>
      </div>
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl md:text-6xl font-bold text-gray-900 mb-6">
            Your Next <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-emerald-800">Adventure Awaits</span>
          </h2>
          <p className="text-gray-600 max-w-2xl mx-auto text-lg">
            Limited spots available for these confirmed departures. Book now to secure your place in these epic adventures.
          </p>
        </div>
                ...
                <div className="relative w-96 h-96">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="relative w-48 h-48">
                      <Mountain className="w-full h-full text-emerald-300/20" />
                    </div>
                  </div>
                </div>
              </div>
